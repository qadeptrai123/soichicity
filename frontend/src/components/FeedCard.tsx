...
// Helper: Sort media items - videos first (including YouTube)
...
const Gallery: React.FC<GalleryProps> = ({ items }) => {
    ...
    const isMultipleItems = sortedItems.length > 1;
                    ...
                    <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex gap-1 z-10">
                        {sortedItems.map((_, index) => (
                            <button
                                ...
                                className={`w-2 h-2 rounded-full transition-all ${index === selectedIndex ? 'bg-white w-6' : 'bg-white/50 hover:bg-white/75'
                                    }`}
                            />
                        ))}
                    </div>
